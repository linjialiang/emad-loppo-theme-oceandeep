...
(function () {
  ...
  if (firstH2Title && EMAD_LOPPO.article_toc && h2TitleNumber.length >= 2) {
    ...
    toc_div.innerHTML = '<h3>Contents</h3>' + EMAD_LOPPO.article_toc;
...
function insert_icon_image(iconName) {
  ...
  iconsArr.forEach(function (i) {
    ...
    if (iconName === undefined) {
      img.src = EMAD_LOPPO.relative_root_path + 'assets/css/' + i.dataset.icon + '.svg';
    } else {
      img.src = EMAD_LOPPO.relative_root_path + 'assets/css/' + iconName + '.svg';
    }
    i.appendChild(img);
  });
}
